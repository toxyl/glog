...
func demoTables() {
	...
	tbl := glog.NewTable(
		glog.NewTableColumnLeft("Left").Push(
			10,
			" ",
			"---",
		...
		glog.NewTableColumnCenter("Center").Push(
			false,
			"my little pony",
			50,
			60,
		...
		glog.NewTableColumnRight("Right").Push(
			10,
			20,
			true,
			[]int{40, 50},
		...
		glog.NewTableColumnCenterCustom("Pad Char", '∙', nil).Push(
			-10,
		...
		glog.NewTableColumnCenterCustom("No Highlight", ' ', fmt.Sprint).Push(
			-10,
			" ",
			"---",
